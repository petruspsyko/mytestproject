...
text_1 = visual.TextStim(win=win, name='text_1',
    text='Ange vilken hand du föredrar att använda för var och en av aktiviteterna nedan genom att sätta + i rätt kolumn. I de fall där du har en stark preferens för att använda den ena handen framför den andra, så till den grad att du aldrig skulle använda den andra handen om du inte absolut måste, sätt ++. I de fall där det inte spelar någon roll vilken hand du använder, sätt + ibåda kolumnerna. ',
    font='Arial',
...
while continueRoutine:
    ...
    # *key_resp_1* updates
    waitOnFlip = False
    if key_resp_1.status == NOT_STARTED and tThisFlip >= 6-frameTolerance:
        # keep track of start time/frame for later
